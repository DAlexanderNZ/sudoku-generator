...
def is_valid(grid, num, pos, board_size=9):
...
def generate_sudoku(board_size=9):
    """ 
...
def check_unique_solution(grid, last_pos,  last_removed):
    """ 
    ...
    for num in range(1, grid.shape[0] + 1):
        if num != last_removed and is_valid(grid, num, last_pos):
            return False
    return True

def redact_sudoku(grid, difficulty=0.5):
    """ 
    ...
    while not successfull_redactions:
        ...
        while redacted_cells <= (total_cells - cells_to_keep):
            ...
            if redacted_grid[i][j] != 0:
                ...
                redacted_grid[i][j] = 0
                redacted_cells += 1
                if not check_unique_solution(redacted_grid, [i, j], last_removed):
                    redacted_cells -= 1
...
if __name__ == "__main__":
    sudoku9 = generate_sudoku()
    redacted_sudoku9 = redact_sudoku(sudoku9, 0.5)
    print("\n9x9 Grid:")
    print_sudoku(sudoku9)
    print("\nRedacted 9x9 Grid:")
    print_sudoku(redacted_sudoku9)